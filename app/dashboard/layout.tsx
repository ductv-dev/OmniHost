...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-dvh bg-zinc-100 text-zinc-950 dark:bg-zinc-950 dark:text-zinc-50">
      <div className="mx-auto flex min-h-dvh w-full max-w-130 flex-col bg-white shadow-2xl shadow-zinc-950/5 dark:bg-zinc-950">
        ...
        <nav className="fixed bottom-0 left-1/2 z-50 w-full max-w-130 -translate-x-1/2 border-t border-zinc-200 bg-white/95 px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/95">
            ...
            {navItems.map(item => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`relative flex min-h-14 flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2 transition-colors active:scale-95 ${
                    isActive
                ...
                >
                  <item.icon
                    className={`size-5 transition-colors ${isActive ? 'text-zinc-950 dark:text-white' : 'text-zinc-500'}`}
                  />
                  <span
                    className={`text-[11px] font-semibold leading-none transition-colors ${isActive ? 'text-zinc-950 dark:text-white' : 'text-zinc-500'}`}
                  >
                    {item.label}
                  </span>
                </Link>
